...
/**
 * Frames are transmitted as: [u32 payloadLength][payload bytes]
 * This is robust against chunk boundaries because the browser waits for the exact payload size.
 */
class LengthPrefixedTransformer {
    constructor(expectedBytes = null) {
    ...
    setBytesToWait(expectedBytes) {
        this.expectedBytes = expectedBytes;
    }

    setStartSequence() {
        // Kept for compatibility with the older web app code. No-op for the length-prefixed protocol.
    }
    ...
    async transform(chunk, controller) {
        ...
        while (this.buffer.length >= 4) {
            ...
            if (this.expectedBytes !== null && payloadLength !== this.expectedBytes) {
                console.error(`🚫 Expected ${this.expectedBytes} payload bytes, got ${payloadLength}. Dropping frame.`);
                this.buffer = new Uint8Array(0);
                return;
            }
            ...
            controller.enqueue(this.convertBytes(payload));
        }
    }
...
class ImageDataTransformer extends BytesWaitTransformer {
    ...
    /**
     * Sets the exact number of bytes to wait for one frame.
     * @param {number} expectedBytes - Number of raw bytes for one complete frame.
     */
    setExpectedBytes(expectedBytes) {
        this.waitBytes = expectedBytes;
    }
    ...
    convertBytes(bytes) {
        ...
        if(this.filter){
            this.filter.applyFilter(pixelData, this.width, this.height);
        }
