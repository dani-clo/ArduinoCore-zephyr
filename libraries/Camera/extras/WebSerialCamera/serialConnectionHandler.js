...
class SerialConnectionHandler {
    ...
    async connectSerial() {
        try {
            ...
            console.error('💣 Error occurred while connecting: ' + error.message);
            return false;
        }
    }
    ...
    async requestFrame() {
        ...
        console.log('📸 Requesting frame from the board...');
     ...
     */
    async requestConfig() {
        console.log('⚙️ Requesting camera configuration from the board...');
     ...
     */
    async getConfig() {
        if (!this.currentPort) return STATIC_CAMERA_CONFIG;

        // CameraCaptureRawBytes.ino does not send a config packet.
        // The app uses a fixed static configuration compatible with that sketch.
        return STATIC_CAMERA_CONFIG;
    }
